...
const SITE_CONFIG = {
  ...
  "Data AP": { url: "img/logo-ap.png", height: "60px" },
  "Data TF": { url: "img/logo-ap.png", height: "60px" },
...
function resetDashboard() {
  ...
  var emptyIds = ["sources-center", "sources-legend", "devices-cards"];
  emptyIds.forEach(id => { var el = document.getElementById(id); if(el) el.innerHTML = ""; });
...
function updateDashboard() {
  ...
  var rows = getRowsForSheet(sheetName);
  var cleanPeriod = periodValue.replace("year-", "").replace("month-", "");
...
function updateMonthlyView(sheetName, rows, ym) {
  ...
  updateDevicesCards(current ? {
    mobile: Number(current.smartphone||0),
    desktop: Number(current.ordinateurs||0),
    tablet: Number(current.tablettes||0)
  } : null);
...
function updateYearlyView(sheetName, rows, year) {
  ...
  updateDevicesCards(agg ? {
    mobile: agg.smartphone,
    desktop: agg.ordinateurs,
    tablet: agg.tablettes
  } : null);
...
/* =======================================================
   DOUGHNUT CHARTS (NEW) - SANS CENTRE + PCT EN GRAS
   ======================================================= */
function renderPieCommon(id, sheetName, data, labels, colorCount, centerId, legendId) {
  var canvas = document.getElementById(id);
  if (!canvas) return;
  var ctx = canvas.getContext("2d");
  
  var colors = getAllColors(sheetName).slice(0, colorCount);
  ...
  if (legendEl) {
    var items = data.map((v, i) => ({v, i, l: labels[i], c: colors[i]})).sort((a,b) => b.v - a.v);
    var html = '';
    items.forEach(item => {
      if(item.v > 0) {
        var pct = (item.v / tot * 100).toFixed(0) + "%";
        var rawVal = formatNumber(item.v);
        html += `
          <div class="legend-item">
            <div class="legend-label-group">
              <span class="legend-dot" style="background-color: ${item.c}"></span>
              <span>${item.l}</span>
            </div>
            <div class="legend-value">${pct} <span class="legend-pct">(${rawVal})</span></div>
          </div>
        `;
      }
    });
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { 
          callbacks: { 
            label: function(ctx) { 
              return ' '+ctx.label+' : '+formatNumber(ctx.parsed)+' ('+(tot>0?(ctx.parsed/tot*100):0).toFixed(1)+'%)'; 
            } 
          } 
        }
      }
  ...
  if(id==="sourcesChart") sourcesChart=chart;
}

function renderSourcesPie(sheetName, row) { 
  renderPieCommon("sourcesChart", sheetName, row ? [Number(row.moteurs_de_recherche||0), Number(row.entrees_directes||0), Number(row.sites_externes||0), Number(row.assistants_ia||0), Number(row.reseaux_sociaux||0), Number(row.campagnes||0)] : [], ["Moteurs de recherche", "Entrées directes", "Sites externes", "Assistants IA", "Réseaux sociaux", "Campagnes"], 6, "sources-center", "sources-legend"); 
}
function renderSourcesPieFromAgg(sheetName, agg) { 
  renderPieCommon("sourcesChart", sheetName, agg ? [agg.moteurs_de_recherche, agg.entrees_directes, agg.sites_externes, agg.assistants_ia, agg.reseaux_sociaux, agg.campagnes] : [], ["Moteurs de recherche", "Entrées directes", "Sites externes", "Assistants IA", "Réseaux sociaux", "Campagnes"], 6, "sources-center", "sources-legend"); 
}

/* =======================================================
   DEVICES CARDS (NEW)
   ======================================================= */
function updateDevicesCards(data) {
  var container = document.getElementById("devices-cards");
  if (!container) return;

  var mobileVal = data ? (data.mobile || 0) : 0;
  var desktopVal = data ? (data.desktop || 0) : 0;
  var tabletVal = data ? (data.tablet || 0) : 0;
  var total = mobileVal + desktopVal + tabletVal;
  ...
  var devices = [
    ...
    { 
      label: "TABLETTES", 
      value: tabletVal, 
      icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="transform: rotate(90deg);"><rect x="4" y="2" width="16" height="20" rx="2" ry="2"/><line x1="12" y1="18" x2="12.01" y2="18"/></svg>' 
    }
  ];

  devices.sort(function(a, b) { return b.value - a.value; });

  var html = "";
  devices.forEach(function(dev) {
    var pct = total > 0 ? (dev.value / total * 100).toFixed(0) + "%" : "0%";
    var raw = formatNumber(dev.value);
    html += `
      <div class="device-card">
        <div class="device-icon">${dev.icon}</div>
        <div class="device-value">${pct} <span class="device-raw">(${raw})</span></div>
...
function updateTopPages(sheetName, period) {
  ...
  if (sheetName === "Data RR" || sheetName === "Data TF") { if(block) block.style.display = "none"; return; } else { if(block) block.style.display = "block"; }
  ...
  filtered.forEach(function(r) {
    html += '<tr><td class="position">'+r.position+'</td><td class="evolution '+getEvolutionClass(r.evolution)+'">'+getEvolutionLabel(r.evolution)+'</td><td class="page-title"><a href="'+r.url+'" target="_blank" title="'+(r.titre_page||"")+'">'+(r.titre_page||"")+'</a></td><td class="numeric">'+formatNumber(r.vues)+'</td><td class="numeric">'+(r.pct_trafic*100).toFixed(2)+'%</td><td class="numeric">'+(r.taux_rebond*100).toFixed(1)+'%</td><td class="numeric">'+r.temps_moyen+'</td></tr>';
  });
  tbody.innerHTML = html;
}

/* =======================================================
   TÉLÉCHARGEMENT AVEC NOM COMPLET
   ======================================================= */
...
function toggleDarkMode() { 
  ...
  btn.title = document.body.classList.contains("dark") ? "Passer au mode clair" : "Passer au mode sombre"; 
}

function exportPDF() { window.print(); }
async function refreshData() { var btn=document.querySelector('.refresh-btn'); btn.disabled=true; try { localStorage.removeItem(CACHE_KEY); localStorage.removeItem(CACHE_MONTH_KEY); var res=await fetch(API_URL); allData=await res.json(); setCachedData(allData); initPeriodSelector(); applyHashToSelectors(); updateDashboard(); console.log('Données rafraîchies'); } catch(err){ alert('Erreur rafraîchissement'); } finally { btn.disabled=false; } }